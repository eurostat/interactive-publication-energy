...
function showWelcome() {
    $("#welcomeScreen").show();
    $("#indicatorInfos, #randomTxt, #noData, #indicatorsWrapper, #selectorThema, #selectorThemaiPad").css("opacity", 0);
...
function hideWelecome() {
    ...
    $("#indicatorInfos, #randomTxt, #noData, #svg_vis, #indicatorsWrapper, #vis, #selectorThema, #selectorThemaiPad").fadeTo("slow", 1, function() {
...
BubbleChart = (function() {
    function BubbleChart(data) {
        ...
        this.xPosition_scale = d3.scale.pow().exponent(0.5).domain([0, this.max_amount]).range([0, 1024]);
        this.create_nodes();
        this.create_vis();
        this.emptyBubble = 0;
        this.graphX = null;
                ...
                $(".indicator").click(function() {
                    ...
                    chart.indicators.forEach((function(_this) {
                        if (_this.indCode == strClass) {
							//alert("click indicator  "+strClass);
                            chart.selectedIndicator = _this;
							var index = parseInt($(this).index());
							console.log(thema);
                            if (true) {

								rootUrl = "http://ec.europa.eu/eurostat/wdds/rest/data/v2.1/json/en/nrg_100a?precision=1&geo=AT&geo=BE&geo=BG&geo=CY&geo=CZ&geo=DE&geo=DK&geo=EE&geo=EL&geo=ES&geo=EU28&geo=FI&geo=FR&geo=HR&geo=HU&geo=IE&geo=IT&geo=LT&geo=LU&geo=LV&geo=MT&geo=NL&geo=PL&geo=PT&geo=RO&geo=SE&geo=SI&geo=SK&geo=UK&product=0000&product=2000&product=3000&product=4000&product=5100&product=5500&unit=KTOE&indic_nrg=B_100900&time=2014&unitLabel=label";
                               //rootUrl = "http://ec.europa.eu/eurostat/wdds/rest/data/v1.1/json/en/tps00191?citizen=TOTAL&sex=T&unit=PER&time=2014&age=TOTAL&asyl_app=ASY_APP&precision=2";
							    //alert(_this.indCode);
                                JSONstat(rootUrl,
                                    function() {
                                        var dataset = this;
                                        if (dataset.Dataset(0) == null) {
                                            ...
                                            paramObj.geo = "EU28";
											//alert(thema.product);
											paramObj.product = thema.product;
											paramObj.unit= "KTOE";
											paramObj.indic_nrg= "B_100900";
											 
											var valueEU = dataset.Dataset(0).Data(paramObj).value;
                                            chart.data.forEach((function(elem) {
                                                var code = elem["countryCode"].toUpperCase();
												
                                                paramObj = {},
                                                paramObj.geo = code;
												paramObj.product = "0000";
												paramObj.unit= "KTOE";
												paramObj.indic_nrg= "B_100900";
												
												var valAllProd = dataset.Dataset(0).Data(paramObj).value;
                                                ...
                                                var val = dataset.Dataset(0).Data(paramObj).value;
												val = (val/valAllProd)*100;
					
                                                if (val == undefined || val == null) {
                                                    elem[strClass] = "NA";
                                                } else {
                                                    //console.log(dataset.Dataset(0).Data(paramObj).value);
                                                    if (_this.indDivider > 0) {
                                                        elem[strClass] = val / _this.indDivider;
                                                    } else {
                                                        elem[strClass] = val;
                                                    }
                                                }
                                        ...
                                        $("#shareDisplay").webuiPopover({
                                            title: chart.translations['ie18nLinkShare'],
                    ...
                    var remainingSize = 768 - 80 - 50 - 10 - (positionXLastElem - positionXFirstElem) - 60;
